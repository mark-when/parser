...
2023-05-01: this`);
    ...
    expect(mw.events.children[1].textRanges.whole.to).toBe(322);
  });
...
2025-01-30: hello`;
    ...
    expect(group.textRanges.whole.from).toBe(35);
    expect(group.textRanges.whole.to).toBe(s.length);
  });

  test("section ranges 2b", () => {
    const s = `# Section
now: this is podracing 
...
`);
    ...
    const section = mw.events.children[0] as EventGroup;
    // style defaults to 'group' when not specified
...
`);
  ...
  test.each(sp())(
    "jumping from deep level to shallow closes all intermediate",
    (p) => {
      const mw = p(`
# Level 1
## Level 2
...
`);
  ...
  test.each(sp())(
    "section with only nested sections (no direct events)",
    (p) => {
      const mw = p(`
# Parent
## Child 1
